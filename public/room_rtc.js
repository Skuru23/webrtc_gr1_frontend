...
const roomid = urlParam.get("room");
...
function checkAuth() {
  fetch(`http://localhost:8000/callroom/getOne/${roomid}`, {
    ...
    .then((data) => {
      ...
      roomPart = data.participants;
      //console.log(roomPart);
      console.log(data.registor);
      const userData = JSON.parse(user);
      console.log(userData._id);
      if (
        !roomPart.includes(userData.username) &&
        data.registor != userData._id
      ) {
        alert("U are not allowed to join this room");
...
export { channel, rtmClient, displayName, isAllowed };
